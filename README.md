...
> **关于 Reader（普通终端用户）**：上面这套 IAM 模型是给 **Publisher/Curator/Admin** 这种少量、可信、有 AWS 凭据的角色用的。**业务用户、分析师、客服坐席**这种大规模、不能持有 IAM 凭据的人群，走的是 **Cognito User Pool + Identity Pool** 的 JWT-换-临时-凭据路径——他们机器上零 AWS 配置文件，靠 OS keyring 持有凭据，体验是"打开浏览器登录一次，30 天透明可用"。详见 [docs/10-end-user-access.md](docs/10-end-user-access.md)。
...
│   │   └── scripts/publish.py
│   └── skill-cli/                     # 终端用户的 JWT→IAM 桥接 CLI
│       ├── skill_cli.py
│       ├── pyproject.toml
│       └── README.md
